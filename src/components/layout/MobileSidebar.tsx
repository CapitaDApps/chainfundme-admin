...
import { FaBars, FaChartLine, FaTelegramPlane } from "react-icons/fa";
...
export default function MobileSidebar() {
  ...
  return (
    <Sheet>
      <SheetTrigger asChild className="relative right-3 z-30">
        <FaBars className="text-xl text-white" />
      </SheetTrigger>
      <SheetContent
        side="left"
        className="w-[200px] flex flex-col justify-between  bg-primary-bg border-none  p-0  lg:hidden"
      >
        <SheetHeader>
          <div>
            {" "}
            <SheetTitle className="flex items-center pt-6 pl-2">
              <Image
                width={35}
                height={35}
                alt="capita_logo"
              ...
              />
              <div className=" ">
                <h2 className="font-bold font-sono text-xs text-primary-text">
                  ChainFundMe
                </h2>
            ...
            <div className="flex flex-col gap-3 pt-8 space-y-2">
              <div className="space-y-0.5">
                {links.map((li) => (
                  <SheetClose key={li.title} asChild>
                    <Link
                      href={li.route}
                      key={li.title}
                      className={`${
